...
class Banner extends Component {
  ...
  checkAuth = () => { //check if auth is still valable
    date = new Date();
    if ((!cookies.get("expires_at") | cookies.get("expires_at") < date.getTime() / 1000) && this.props.location.pathname !== "/oauthend") {
  ...
  render() {
    return (
      <nav className="navbar fixed-top navbar-expand-xl navbar-dark col-lg-12">
        <a className={"navbar-brand" + (this.props.history.location.pathname == "/found" ? " active" : "")} onClick={() => this.handlePageChange("")}><img src={logopalccontour} width="40" height="40" class="img-fluid" alt="Responsive image"
