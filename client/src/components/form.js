...
class Form extends Component { //parent component
  ...
  handleSubmit(event) { // check if the ads fullfil the requirements
    if (this.state.title == "" | this.state.description == "") {
      ...
      this.props.putDataToDB(this.state).then(() => { setTimeout(() => this.props.history.push("/all"), 400); loadReCaptcha({ key: "6LcpTZAUAAAAAAFSVV4wHy98dnjHW8Ylf-YIC9OR", id: "reCaptcha" }); this.killReCaptchaBadge() });
    };
  }
  ...
  render() {
    ...
    let type = <PostType updateParent={this.updateParent} type={this.props.type} reward={this.props.reward}/>;
