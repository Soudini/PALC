...
export default class Perso extends Component {
  ...
  searchDataFromDb = (searchText) => {
    axios.post("localhost:3001/api/searchData", {search : { "$or":[{"type":{"$regex": searchText, "$options" : "i"}},{"author":{"$regex": searchText, "$options" : "i"}},{"reward":{"$regex": searchText, "$options" : "i"}},{"description":{"$regex": searchText, "$options" : "i"}} ]}})
